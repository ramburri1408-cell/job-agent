"""
Job Scraper — scrapes LinkedIn, Indeed, Dice for new listings.
Saves results to data/jobs.json, deduplicating by job ID.
"""
...
def scrape_dice(query: str) -> list[dict]:
    """Scrape Dice job listings via their JSON API."""
    jobs = []
    url = (
...
def scrape_remotive(query: str) -> list[dict]:
    """Scrape Remotive (remote jobs board, bot-friendly API)."""
    jobs = []
    try:
        ...
        for item in data.get("jobs", []):
            ...
            jobs.append({
                ...
                "scraped_at": now_iso(),
                "status": "new",
                "recruiter_email": item.get("company_email", ""),
                "recruiter_name": "",
                "tailored_resume": None,
                "email_draft": None,
                "email_sent": False,
